...
import { CloakSessionManager, deriveFingerprintSeed } from './sessions.js';
/**
 ...
 */
export async function resolveCloakBrowser(options) {
    const explicitBinaryPath = options.executablePath || process.env.CLOAKBROWSER_PATH || process.env.CLOAKBROWSER_BINARY_PATH;
    const configuredMode = options.binaryMode || process.env.CLOAKBROWSER_BINARY_MODE;
    ...
    return {
        binaryPath: launchOpts.executablePath,
        stealthArgs: (launchOpts.args || []),
        mode,
    };
}
/**
...
function findPidByPort(port) {
    try {
        if (process.platform === 'win32') {
            const out = execSync(`netstat -ano | findstr "LISTENING" | findstr ":${port} "`, { encoding: 'utf-8', timeout: 3000, windowsHide: true }).trim();
            ...
            const out = execSync(`lsof -ti :${port} -sTCP:LISTEN 2>/dev/null || true`, { encoding: 'utf-8', timeout: 3000 }).trim();
...
export class CloakBrowserProvider {
    sessionManager = new CloakSessionManager();
    /** Map of port → PID for CloakBrowser instances we launched */
    managedPorts = new Map();
    getSessionManager() {
        return this.sessionManager;
    }
    async isCDPReady(host = '127.0.0.1', port = 9222) {
    ...
    async startBrowser(options = {}) {
        ...
        throw new Error(`Timed out waiting for CloakBrowser CDP on ${host}:${port} after ${timeout}ms.\n` +
            `Binary: ${binaryPath}`);
    }
    async fetchWebSocketDebuggerUrl(host, port) {
        return new Promise((resolve) => {
